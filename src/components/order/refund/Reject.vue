<template>
  <el-dialog title="拒绝退款" :visible.sync="dialogVisible" width="40%" @open="openCallback" @close="closeCallback"
             loading="loading">
    <el-form ref="formReject" :model="form" :rules="rules">
        ...
        this.$confirm('拒绝后，买家将不能发起退款，请确认理由充分！', '拒绝退款？', {
          ...
          this.$refs.formReject.validate(async (valid) => {
            if (valid) {
              this.loading = true
              try {
                const resResult = await RefundApi.reject(this.oiid, this.form.text, this.form.remark)
                this.loading = false

<template>
  <div>
    <el-form ref="modelForm" label-width="120px" label-position="right" :model="model"
             :rules="rules">
      <el-form-item label="限时折扣名称" prop="name">
        <el-input class="name" v-model="model.name" :maxlength="4" placeholder="最多4个字"></el-input>
      </el-form-item>
      <el-form-item label="折扣持续时间" prop="timeRange">
        <el-date-picker
          ref="timePicker"
      ...
      <el-form-item  label-width="0">
        ...
        <el-button type="primary" @click="_submitForm" :loading="loading">确 定</el-button>
      </el-form-item>
    </el-form>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      const validatTime = (rule, value, callback) => {
        if (!this.timeRange) {
          callback(new Error('折扣持续时间不能为空'))
          return
        }
      ...
      return {
        ...
        rules: {
          ...
          timeRange: [
            {validator: validatTime, trigger: 'blur'}
          ],
          count: [
      ...
      async _initDiscountForm() {
        try {
          ...
          this.model = {
            ...
            items: res.data.items.filter((item) => { return item.product })
          }
          const start = new Date()
          const end = new Date()
      ...
      _createdDiscount() {
        ...
        DiscountApi.create(this.model)
          .then((res) => {
            ...
            this.$eventBus.emit('createdDiscount')
            this.$router.back()
          })
          .catch((err) => {
            this.loading = false
